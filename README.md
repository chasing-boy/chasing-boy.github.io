...
一位数码管通常10个引脚：2个COM公共端，a、b...g、dp共8个引脚。



连接：将数码管的其中一个COM引脚通过一只限流电阻连接ESP32引脚3.3V，其余数码管引脚连接esp32对应引脚

```c++
//实验使用共阳数码管

//将数码管引脚与ESP32对应引脚接起来，COM通过一只限流电阻连接ESP引脚3.3V
//数码管引脚与对应esp32引脚连接(数码管led-a单元与esp32-4号引脚连接)
int pin_a = 4;
int pin_b = 5;
int pin_c = 19;
int pin_d = 21;
...
即4个1位数码管并列集中在一起形成的一体的数码管。

相同的地方是数码管中LED的分段映射相同，也就是段选线相同。

有区别的地方是1位数码管有两个相同的公共CM端接地/接电源，而**4位数码管没有公共端**，有4个控制不同位置显示的选通端。

- 可以控制多位中哪一位的数码管叫作`位选线`，负责显示数字的段线(a-dp)叫作`段选线`。

4位数码管：4个位选线，8个段选线。判断共阴还是共阳同1位数码管相同。
...
通过对位选线进行扫描，使4位数码管看起来好像同时显示4位数字。
...
```c++
//动态扫描 同时显示4位数字 可以传入0-9999
void display_4_number(int num){
  //获取每位数字
  for(int i=3;i>=0;i--){
    display_number(i,num%10);
    num /= 10;
    delay(5);//快速扫描
  }
}
